(function (global) {
  ...
  const CRAWLER_STATUS_REFRESH_INTERVAL = 5000;
  let crawlerPollTimer = null;
  let crawlerPollInFlight = false;
  ...
  const stopCrawlerPolling = () => {
    if (crawlerPollTimer !== null) {
      if (typeof global.clearInterval === 'function') {
        global.clearInterval(crawlerPollTimer);
      }
      crawlerPollTimer = null;
    }
    crawlerPollInFlight = false;
  };

  const pollStatusTick = async () => {
    if (crawlerPollInFlight) return;
    if (!global.currentProject) return;
    if (typeof document !== 'undefined' && document.hidden) return;
    crawlerPollInFlight = true;
    try {
      await pollStatus();
    } catch (error) {
      console.error('crawler_poll_failed', error);
    } finally {
      crawlerPollInFlight = false;
    }
  };
  ...
  const startCrawlerPolling = () => {
    ...
    void pollStatusTick();
    crawlerPollTimer = global.setInterval(() => {
      void pollStatusTick();
    }, CRAWLER_STATUS_REFRESH_INTERVAL);
  };
  ...
  const init = () => {
    ...
    if (typeof global.addEventListener === 'function') {
      global.addEventListener('beforeunload', stopCrawlerPolling);
    }
    ...
    global.onAdminProjectChange = handleProjectChange;
    if (global.currentProject) {
      handleProjectChange(global.currentProject);
    }
    ...
    global.startCrawlerPolling = startCrawlerPolling;
    global.stopCrawlerPolling = stopCrawlerPolling;
    try {
