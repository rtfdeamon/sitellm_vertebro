(function (global) {
  ...
  function buildInfo(status) {
    if (!status) {
      return 'Выберите проект, чтобы управлять Telegram ботом.';
    }
    const parts = [];
  ...
  function applyProject(project) {
    ...
    if (!project || !project.name) {
      ...
      infoLabel.textContent = 'Сохраните токен и запустите бота.';
      updatePlaceholder(null);
      updateButtons(null);
    }
  }
  ...
  if (saveButton) {
    saveButton.addEventListener('click', () => {
      ...
      send('/telegram/config', payload, 'Сохранено');
    });
  }
  ...
  if (startButton) {
    startButton.addEventListener('click', () => {
      ...
      send('/telegram/start', payload, 'Запущено');
    });
  }
  ...
  if (stopButton) {
    stopButton.addEventListener('click', () => {
      ...
      send('/telegram/stop', payload, 'Остановлено');
    });
  }
