(function (global) {
  ...
  const tokenInput = global.document.getElementById('projectMaxToken');
  const autoStartInput = global.document.getElementById('projectMaxAutoStart');
  const statusLabel = global.document.getElementById('projectMaxStatus');
  const infoLabel = global.document.getElementById('projectMaxInfo');
  const messageLabel = global.document.getElementById('projectMaxMessage');
  const saveButton = global.document.getElementById('projectMaxSave');
  const startButton = global.document.getElementById('projectMaxStart');
  const stopButton = global.document.getElementById('projectMaxStop');
  ...
  const translate = (key, fallback = '', params = undefined) => {
    if (typeof global.t === 'function') {
      try {
        ...
        console.warn('max_translate_failed', error);
      }
    }
    if (fallback) {
  ...
  function buildInfo(status) {
    if (!status) {
      return translate('integrationMaxSelectProject', 'Select a project to manage the MAX bot.');
    }
    const parts = [];
    if (status.token_set) {
      parts.push(
  ...
  function applyProject(project) {
    ...
    if (autoStartInput) {
      autoStartInput.checked = !!project?.max_auto_start;
    }
    if (!project || !project.name) {
      ...
      infoLabel.textContent = translate(
        ...
        'Save the token and start the MAX bot.',
      );
      updatePlaceholder(null);
      updateButtons(null);
    }
  ...
  if (saveButton) {
    saveButton.addEventListener('click', () => {
      ...
      send('/max/config', payload, translate('projectsSaved', 'Saved'));
    });
  }
  ...
  if (startButton) {
    startButton.addEventListener('click', () => {
      ...
      send('/max/start', payload, translate('integrationActionStarted', 'Started'));
    });
  }
  ...
  if (stopButton) {
    stopButton.addEventListener('click', () => {
      ...
      send('/max/stop', payload, translate('integrationActionStopped', 'Stopped'));
    });
  }
