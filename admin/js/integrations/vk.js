(function (global) {
  ...
  function buildInfo(status) {
    if (!status) {
      return tl('Выберите проект, чтобы управлять VK ботом.');
    }
    const parts = [];
  ...
  function applyProject(project) {
    ...
    if (!project || !project.name) {
      ...
      infoLabel.textContent = tl('Сохраните токен и запустите VK бота.');
      updatePlaceholder(null);
      updateButtons(null);
    }
  }
  ...
  if (saveButton) {
    saveButton.addEventListener('click', () => {
      ...
      send('/vk/config', payload, tl('Сохранено'));
    });
  }
  ...
  if (startButton) {
    startButton.addEventListener('click', () => {
      ...
      send('/vk/start', payload, tl('Запущено'));
    });
  }
  ...
  if (stopButton) {
    stopButton.addEventListener('click', () => {
      ...
      send('/vk/stop', payload, tl('Остановлено'));
    });
  }
