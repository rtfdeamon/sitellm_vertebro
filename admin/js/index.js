...
const LOG_SANITIZE_PATTERN = /llama-cpp-python/i;
...
function applySessionPermissions() {
  ...
  const isSuper = Boolean(session.is_super);
  if (promptsCard) promptsCard.style.display = isSuper ? '' : 'none';
  if (logsCard) logsCard.style.display = isSuper ? '' : 'none';
  if (knowledgeServiceCard) knowledgeServiceCard.style.display = isSuper ? '' : 'none';
...
window.applySessionPermissions = applySessionPermissions;
...
function buildPromptMeta(parts) {
  const meta = [];
  if (parts?.prompt_length) meta.push(`Length: ${parts.prompt_length}`);
  if (parts?.session) meta.push(`Session: ${parts.session}`);
  return meta.join(' · ');
}

function extractPromptInsights(lines) {
  if (!Array.isArray(lines)) {
    return { prompts: [], latestMatch: null };
  }
  const prompts = [];
  let latestMatch = null;
  const activeProject = (typeof currentProject === 'string' ? currentProject : '').trim().toLowerCase();
  lines.forEach((line) => {
    if (!line || !line.includes('llm_prompt_compiled')) return;
    let formatted = line;
    try {
      const payload = JSON.parse(line);
      const preview = (payload.prompt_preview || payload.prompt_base || '').trim();
      const project = (payload.project || payload.mode || '').trim();
      ...
      const docs = knowledge
        ...
        .map(String)
        .slice(0, 6);
      const clipped = preview.length > 280 ? `${preview.slice(0, 277)}…` : preview || formatted;
      const docSuffix = docs.length ? `\n  docs: ${docs.join(', ')}` : '';
      formatted = project ? `[${project}] ${clipped}${docSuffix}` : `${clipped}${docSuffix}`;
      const projectKey = project.toLowerCase();
      if (!activeProject || !projectKey || projectKey === activeProject) {
        latestMatch = {
          text: clipped || formatted,
          docs,
          meta: buildPromptMeta(payload),
        };
      }
    } catch (error) {
      // keep raw line when parsing fails
    }
    prompts.push(formatted);
  });
  return { prompts, latestMatch };
}

function renderLogInfo(linesCount) {
  if (!logInfoElement) return;
  const updatedLabel = t('logsUpdatedLabel', { time: formatTimestamp(Date.now()) });
  logInfoElement.textContent = `${linesCount} · ${updatedLabel}`;
}

function renderLogs(lines) {
  if (!logsElement) return;
  const joined = lines.join('\n');
  if (joined === lastLogsHash) return;
  lastLogsHash = joined;
  logsElement.textContent = joined;
  logsElement.scrollTop = logsElement.scrollHeight;
}

async function fetchAdminLogs({ force } = {}) {
  if (!logsElement || !window.adminSession?.is_super) return;
  if (logsFetchInFlight && !force) return;
  logsFetchInFlight = true;
  try {
    const limitRaw = Number(logLimitInput?.value || 200);
    const limit = Number.isFinite(limitRaw) ? Math.min(Math.max(Math.trunc(limitRaw), 50), 1000) : 200;
    const resp = await fetch(`/api/v1/admin/logs?limit=${limit}`);
    if (resp.status === 401) {
      if (logsCard) logsCard.style.display = 'none';
      if (logInfoElement) {
        logInfoElement.textContent = t('logsAccessDenied') || 'Logs are available only to super administrators.';
      }
      if (promptLogsElement) promptLogsElement.textContent = '';
      if (logsElement) logsElement.textContent = '';
      lastLogsHash = '';
    ...
    const filtered = filterRecentLines(rawLines).filter((line) => !LOG_SANITIZE_PATTERN.test(line));
    renderLogs(filtered);
    renderLogInfo(filtered.length);
    if (promptLogsElement) {
      const { prompts, latestMatch } = extractPromptInsights(filtered);
      const content = prompts.join('\n\n');
      if (promptLogsElement.textContent !== content) {
        promptLogsElement.textContent = content;
        if (content) {
          promptLogsElement.scrollTop = promptLogsElement.scrollHeight;
        }
      }
      if (latestMatch) {
    ...
    if (logInfoElement) {
      logInfoElement.textContent = t('logsLoadFailed') || 'Failed to load logs';
    }
  } finally {
...
function startLogsPolling({ immediate = true } = {}) {
  if (!window.adminSession?.is_super || !logsElement) {
    stopLogsPolling();
    return;
  }
  if (logsPollTimer) return;
  if (immediate) {
    fetchAdminLogs({ force: true }).catch((error) => console.error('admin_logs_initial_failed', error));
  }
...
if (logLimitInput) {
  logLimitInput.addEventListener('change', () => {
    fetchAdminLogs({ force: true }).catch((error) => console.error('admin_logs_limit_failed', error));
  });
}
